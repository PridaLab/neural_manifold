...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  1 15:06:48 2022
...
#%% GENERAL PARAMS
save_dir =  '/media/julio/DATOS/spatial_navigation/Jercog_data/LT/results/place_cells_study'
...
for fname in fname_list:
    ...
    spikes_signal = np.concatenate(pd_struct["ML_spikes"].values, axis= 0)
    ...
    M2019_place_cells_study[fname] = pc.get_place_cells(pos_signal, spikes_signal, vel_signal = vel_signal,
                          direction_signal = direction_signal, mouse = fname, **params)
   
    save_ks = open(os.path.join(params["save_dir"], "M2019_place_cells_study.pkl"), "wb")
    pickle.dump(M2019_place_cells_study, save_ks)
    save_ks.close()
    
#%% M2021: 
#load data
file_dir = '/media/julio/DATOS/spatial_navigation/Jercog_data/LT/2021'
M2021 = gu.load_files(file_dir, '*_PyalData_struct.mat', verbose = True, struct_type = "PyalData")

fname_list = list(M2021.keys())
M2021_place_cells_study = dict()
for fname in fname_list:
    ...
    M2021_place_cells_study[fname] = pc.get_place_cells(pos_signal, spikes_signal, vel_signal = vel_signal,
                          direction_signal = direction_signal, mouse = fname, **params)
   
    save_ks = open(os.path.join(params["save_dir"], "M2021_place_cells_study.pkl"), "wb")
    pickle.dump(M2021_place_cells_study, save_ks)
    save_ks.close()
    
#%% M2022: 
#load data
file_dir = '/media/julio/DATOS/spatial_navigation/Jercog_data/LT/2022'
M2022 = gu.load_files(file_dir, '*_PyalData_struct.mat', verbose = True, struct_type = "PyalData")

fname_list = list(M2022.keys())
M2022_place_cells_study = dict()
for fname in fname_list:
    ...
    M2022_place_cells_study[fname] = pc.get_place_cells(pos_signal, spikes_signal, vel_signal = vel_signal,
                          direction_signal = direction_signal, mouse = fname, **params)
   
    save_ks = open(os.path.join(params["save_dir"], "M2022_place_cells_study.pkl"), "wb")
    pickle.dump(M2022_place_cells_study, save_ks)
    save_ks.close()
    
#%% M2023: 
#load data
file_dir = '/media/julio/DATOS/spatial_navigation/Jercog_data/LT/2023'
M2023 = gu.load_files(file_dir, '*_PyalData_struct.mat', verbose = True, struct_type = "PyalData")

fname_list = list(M2023.keys())
M2023_place_cells_study = dict()
for fname in fname_list:
    ...
    M2023_place_cells_study[fname] = pc.get_place_cells(pos_signal, spikes_signal, vel_signal = vel_signal,
                          direction_signal = direction_signal, mouse = fname, **params)
   
    save_ks = open(os.path.join(params["save_dir"], "M2023_place_cells_study.pkl"), "wb")
    pickle.dump(M2023_place_cells_study, save_ks)
    save_ks.close()
    
#%% M2024: 
#load data
file_dir = '/media/julio/DATOS/spatial_navigation/Jercog_data/LT/2024'
M2024 = gu.load_files(file_dir, '*_PyalData_struct.mat', verbose = True, struct_type = "PyalData")

fname_list = list(M2024.keys())
M2024_place_cells_study = dict()
for fname in fname_list:
    ...
    M2024_place_cells_study[fname] = pc.get_place_cells(pos_signal, spikes_signal, vel_signal = vel_signal,
                          direction_signal = direction_signal, mouse = fname, **params)
   
    save_ks = open(os.path.join(params["save_dir"], "M2024_place_cells_study.pkl"), "wb")
    pickle.dump(M2024_place_cells_study, save_ks)
    save_ks.close()
    
#%% M2025: 
#load data
file_dir = '/media/julio/DATOS/spatial_navigation/Jercog_data/LT/2025'
M2025 = gu.load_files(file_dir, '*_PyalData_struct.mat', verbose = True, struct_type = "PyalData")

fname_list = list(M2025.keys())
M2025_place_cells_study = dict()
for fname in fname_list:
    ...
    M2025_place_cells_study[fname] = pc.get_place_cells(pos_signal, spikes_signal, vel_signal = vel_signal,
                          direction_signal = direction_signal, mouse = fname, **params)
   
    save_ks = open(os.path.join(params["save_dir"], "M2025_place_cells_study.pkl"), "wb")
    pickle.dump(M2025_place_cells_study, save_ks)
    save_ks.close()
    
#%% M2026: 
#load data
file_dir = '/media/julio/DATOS/spatial_navigation/Jercog_data/LT/2026'
M2026 = gu.load_files(file_dir, '*_PyalData_struct.mat', verbose = True, struct_type = "PyalData")

fname_list = list(M2026.keys())
M2026_place_cells_study = dict()
for fname in fname_list:
    ...
    M2026_place_cells_study[fname] = pc.get_place_cells(pos_signal, spikes_signal, vel_signal = vel_signal,
                          direction_signal = direction_signal, mouse = fname, **params)
   
    save_ks = open(os.path.join(params["save_dir"], "M2026_place_cells_study.pkl"), "wb")
    pickle.dump(M2026_place_cells_study, save_ks)
    save_ks.close()
