...
for mouse in miceList:
    ...
    for ncell_idx, ncell_val in enumerate(cell_list):
        ...
        saveFile = open(os.path.join(save_dir, 'ncells_results_dict.pkl'), "wb")
...
ncells_dict = load_pickle(data_dir, 'ncells_results_dict.pkl')
...
for mouse in miceList:
    ...
    umap_dec.append(np.nanmean(ncells_dict[mouse]['dec_dict']['umap'][:,:,:,0,0],axis=(1,2)))
    og_dec.append(np.nanmean(ncells_dict[mouse]['dec_dict']['clean_traces'][:,:,:,0,0],axis=(1,2)))

    inner_dim_abids.append(np.nanmean(ncells_dict[mouse]['inner_dim']['abids'],axis=1))
    inner_dim_tle.append(np.nanmean(ncells_dict[mouse]['inner_dim']['tle'],axis=1))
    inner_dim_mom.append(np.nanmean(ncells_dict[mouse]['inner_dim']['mom'],axis=1))

    umap_dim_trust.append(np.nanmean(ncells_dict[mouse]['umap_dim']['trustDim'],axis=1))
    umap_dim_cont.append(np.nanmean(ncells_dict[mouse]['umap_dim']['contDim'],axis=1))
    umap_dim_hmean.append(np.nanmean(ncells_dict[mouse]['umap_dim']['hmeanDim'],axis=1))

    umap_SI_pos.append(np.nanmean(ncells_dict[mouse]['SI_dict']['umap']['pos']['sI'][:,:],axis=1))
    umap_SI_dirmat.append(np.nanmean(ncells_dict[mouse]['SI_dict']['umap']['dir_mat']['sI'][:,:],axis=1))
    umap_SI_time.append(np.nanmean(ncells_dict[mouse]['SI_dict']['umap']['time']['sI'][:,:],axis=1))
    umap_SI_vel.append(np.nanmean(ncells_dict[mouse]['SI_dict']['umap']['vel']['sI'][:,:],axis=1))

    og_SI_pos.append(np.nanmean(ncells_dict[mouse]['SI_dict']['clean_traces']['pos']['sI'][:,:],axis=1))
    og_SI_dirmat.append(np.nanmean(ncells_dict[mouse]['SI_dict']['clean_traces']['dir_mat']['sI'][:,:],axis=1))
    og_SI_time.append(np.nanmean(ncells_dict[mouse]['SI_dict']['clean_traces']['time']['sI'][:,:],axis=1))
    og_SI_vel.append(np.nanmean(ncells_dict[mouse]['SI_dict']['clean_traces']['vel']['sI'][:,:],axis=1))
...
m = np.nanmean(umap_dec,axis=1)
sd = np.nanstd(umap_dec,axis=1)
ax.plot(cell_list, m, label='umap')
ax.fill_between(cell_list, m-sd, m+sd, alpha = 0.3)
m = np.nanmean(og_dec,axis=1)
sd = np.nanstd(og_dec,axis=1)
ax.plot(cell_list, m, label='og')
ax.fill_between(cell_list, m-sd, m+sd, alpha = 0.3)
ax.set_xlabel('Num cells')
ax.set_ylabel('Med Abs Error (cm)')
ax.legend()
plt.savefig(os.path.join(data_dir,'nCells_decoders.svg'), dpi = 400,bbox_inches="tight")
plt.savefig(os.path.join(data_dir,'nCells_decoders.png'), dpi = 400,bbox_inches="tight")
