...
#%% M2021
f = open(os.path.join(save_dir,'M2021_umap_ncells_logFile.txt'), 'w')
original = sys.stdout
sys.stdout = gu.Tee(sys.stdout, f)
...
#%% M2024
f = open(os.path.join(save_dir,'M2024_umap_ncells_logFile.txt'), 'w')
original = sys.stdout
sys.stdout = gu.Tee(sys.stdout, f)
...
#%% M2026
f = open(os.path.join(save_dir,'M2026_umap_ncells_logFile.txt'), 'w')
original = sys.stdout
sys.stdout = gu.Tee(sys.stdout, f)
...
ax.plot(dim_space, m[:,0], color = cpal[1], label = 'local')
ax.fill_between(dim_space, m[:,0]-sd[:,0], m[:,0]+sd[:,0], color = cpal[1], alpha=0.3)
ax.plot(dim_space, m[:,4], color = cpal[1], linestyle='--', label = 'global')
ax.fill_between(dim_space, m[:,4]-sd[:,4], m[:,4]+sd[:,4], color = cpal[1], alpha=0.3)
...
ax.plot(dim_space, m[:,0], color = cpal[4], label = 'local')
ax.fill_between(dim_space, m[:,0]-sd[:,0], m[:,0]+sd[:,0], color = cpal[4], alpha=0.3)
ax.plot(dim_space, m[:,4], color = cpal[4], linestyle='--', label = 'global')
ax.fill_between(dim_space, m[:,4]-sd[:,4], m[:,4]+sd[:,4], color = cpal[4], alpha=0.3)
...
#%%
plt.tight_layout()
plt.savefig(os.path.join(save_fig,'umap_dims_both.jpg'), dpi = 400,bbox_inches="tight",transparent=True)
plt.savefig(os.path.join(save_fig,'umap_dims_both.svg'), dpi = 400,bbox_inches="tight",transparent=True)
