...
close all; clear; clc;
%% FILE
data_file =  'CZ4_allo_events_s3.mat';
% Load file
load(data_file);
...
trial_data(1).Inscopix_traces = tracesEvents.traces(1:return_intervals(1,1)-1,:);
...
trial_data(1).cell_idx = 1:size(tracesEvents.traces,2);
...
for return_idx = 1:size(return_intervals,1)
    ...
    trial_data(ii).Inscopix_traces = tracesEvents.traces(st_trial:en_trial,:);
    fields = fieldnames(tracesEvents);
    spike_fields = fields(contains(fields,'spikes_'));
    for idx = 1:length(spike_fields)
