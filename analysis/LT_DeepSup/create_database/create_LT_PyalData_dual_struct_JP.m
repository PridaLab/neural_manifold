close all; clear ; clc;
%%
data_file =  'ThyCalbRCaMP8_rot_events_s4.mat';
%% Load struct
load(data_file);
...
for ii = 1:size(cState_exp,1)-1
    ...
    trial_data(ii).red_raw_traces = tracesEvents.red_raw_traces(cState_exp(ii,1):cState_exp(ii,2),:);
    trial_data(ii).red_denoised_traces = tracesEvents.red_denoised_traces(cState_exp(ii,1):cState_exp(ii,2),:);
    trial_data(ii).green_raw_traces = tracesEvents.green_raw_traces(cState_exp(ii,1):cState_exp(ii,2),:);
    trial_data(ii).green_denoised_traces = tracesEvents.green_denoised_traces(cState_exp(ii,1):cState_exp(ii,2),:);
%     trial_data(ii).conv_traces = tracesEvents.conv_traces(cState_exp(ii,1):cState_exp(ii,2),:);
    ...
    trial_data(ii).green_cell_idx = 1:size(tracesEvents.green_denoised_traces,2);
    trial_data(ii).red_cell_idx = 1:size(tracesEvents.red_denoised_traces,2);
