...
for mouse in mice_list:
    ...
    for case in ['veh', 'CNO']:
        ...
        for emb in ['umap', 'isomap','pca']:
            ...
            #compute centroids
            cent_pre, cent_rot, cent_label, cent_dir = get_centroids(cemb_p, cemb_r, cpos_p[:,0], cpos_r[:,0], 
                                                    cmov_dir_p, cmov_dir_r, num_cent=30)   

            norm_vector_p, cloud_center_p = parametrize_plane(cemb_p)
            norm_vector_r, cloud_center_r = parametrize_plane(cemb_r)

            norm_vector_p = check_norm_vector_direction(norm_vector_p,cloud_center_p, cloud_center_r)
            norm_vector_r = check_norm_vector_direction(norm_vector_r,cloud_center_r, cloud_center_p)

            xlim_emb_p = [np.percentile(cemb_p[:,0],5),np.percentile(cemb_p[:,0],95)]
            ylim_emb_p = [np.percentile(cemb_p[:,1],5),np.percentile(cemb_p[:,1],95)]
            xx_p,yy_p,zz_p = create_plane_from_equation(norm_vector_p, cloud_center_p, xlim_emb_p, ylim_emb_p)


            xlim_emb_r = [np.percentile(cemb_r[:,0],5),np.percentile(cemb_r[:,0],95)]
            ylim_emb_r = [np.percentile(cemb_r[:,1],5),np.percentile(cemb_r[:,1],95)]
            xx_r,yy_r,zz_r = create_plane_from_equation(norm_vector_r, cloud_center_r, xlim_emb_r, ylim_emb_r)

            Ralign = find_rotation_align_vectors(norm_vector_p, -norm_vector_r)  #minus sign to avoid 180 flip

            aligned_norm_vector_r = np.dot(norm_vector_r,Ralign)
            aligned_norm_vector_r = check_norm_vector_direction(aligned_norm_vector_r,cloud_center_r, cloud_center_p)
            axx_r,ayy_r,azz_r = create_plane_from_equation(aligned_norm_vector_r, cloud_center_r, xlim_emb_r, ylim_emb_r)

            aligned_cent_rot = apply_rotation_to_cloud(cent_rot, Ralign,cloud_center_r)
            aligned_cemb_r = apply_rotation_to_cloud(cemb_r, Ralign, cloud_center_r)


            #find axis of rotatio original                                          
            mid_pre = np.median(cemb_p, axis=0).reshape(-1,1)
            mid_rot = np.median(cemb_r, axis=0).reshape(-1,1)
            error = find_rotation(cent_pre-mid_pre.T, cent_rot-mid_rot.T, norm_vector_r.reshape(-1,1), angles)
            norm_error = (np.array(error)-np.min(error))/(np.max(error)-np.min(error))
            rot_angle = np.abs(angles[np.argmin(norm_error)])*180/np.pi
            print(f"\tOriginal {emb}: {rot_angle:2f} degrees")

            #find axis of rotatio aligned                                              
            aligned_mid_rot = np.median(aligned_cemb_r, axis=0).reshape(-1,1)

            aligned_error = find_rotation(cent_pre-mid_pre.T, aligned_cent_rot-aligned_mid_rot.T, aligned_norm_vector.reshape(-1,1), angles)
            aligned_norm_error = (np.array(aligned_error)-np.min(aligned_error))/(np.max(aligned_error)-np.min(aligned_error))
            aligned_rot_angle = np.abs(angles[np.argmin(aligned_norm_error)])*180/np.pi
            print(f"\tAligned {emb}: {aligned_rot_angle:2f} degrees")


            dir_color_p = create_color_mov_dir(cmov_dir_p)
            dir_color_r = create_color_mov_dir(cmov_dir_r)
            cent_dir_color = create_color_mov_dir(cent_dir)
            fig = plt.figure()
            #original embeddings
            ax = plt.subplot(3,3,1, projection = '3d')
            ax.scatter(*cemb_p[:,:3].T, color ='b', s=10)
            ax.scatter(*cemb_r.T, color ='r', s=10)
            ax.set_aspect('equal', adjustable='box')
            ...
            ax.scatter(*cemb_p[:,:3].T, color = dir_color_p, s=10)
            ax.scatter(*cemb_r[:,:3].T, color = dir_color_r, s=10)
            ax.set_aspect('equal', adjustable='box')
            ax = plt.subplot(3,3,2, projection = '3d')
            ax.plot_surface(xx_p, yy_p, zz_p, alpha=0.3, facecolor='b')
            ax.scatter(*cent_pre[:,:3].T, color ='b', s=10)
            ax.plot([cloud_center_p[0], cloud_center_p[0] + 3*norm_vector_p[0]],
                [cloud_center_p[1], cloud_center_p[1] + 3*norm_vector_p[1]],
                [cloud_center_p[2], cloud_center_p[2] + 3*norm_vector_p[2]],
                color='b', linewidth=3)
            ax.plot_surface(xx_r, yy_r, zz_r, alpha=0.3, facecolor='r')
            ax.scatter(*cent_rot[:,:3].T, color ='r', s=10)
            ax.plot([cloud_center_r[0], cloud_center_r[0] + 3*norm_vector_r[0]],
                [cloud_center_r[1], cloud_center_r[1] + 3*norm_vector_r[1]],
                [cloud_center_r[2], cloud_center_r[2] + 3*norm_vector_r[2]],
                color='r', linewidth=3)
            ax.plot_surface(axx_r, ayy_r, azz_r, alpha=0.3, facecolor='orange')
            ax.scatter(*aligned_cent_rot[:,:3].T, color ='orange', s=10)
            ax.plot([cloud_center_r[0], cloud_center_r[0] + 3*aligned_norm_vector_r[0]],
                [cloud_center_r[1], cloud_center_r[1] + 3*aligned_norm_vector_r[1]],
                [cloud_center_r[2], cloud_center_r[2] + 3*aligned_norm_vector_r[2]],
                color='orange', linewidth=3)
            ax.set_aspect('equal', adjustable='box')
            ax = plt.subplot(3,3,5, projection = '3d')
            ax.scatter(*cent_pre[:,:3].T, c = cent_label, s=10, cmap = 'magma')
            ax.scatter(*aligned_cent_rot[:,:3].T, c = cent_label, s=10, cmap = 'magma')
            ax.set_aspect('equal', adjustable='box')
            ax = plt.subplot(3,3,8, projection = '3d')
            ax.scatter(*cent_pre[:,:3].T, color = cent_dir_color, s=10)
            ax.scatter(*aligned_cent_rot[:,:3].T, color = cent_dir_color, s=10)
            ax.set_aspect('equal', adjustable='box')
            #final embeddings
            ax = plt.subplot(3,3,3, projection = '3d')
            ax.scatter(*cemb_p[:,:3].T, color ='b', s=10)
            ax.scatter(*aligned_cemb_r.T, color ='r', s=10)
            ax.set_aspect('equal', adjustable='box')
            ax = plt.subplot(3,3,6, projection = '3d')
            ax.scatter(*cemb_p[:,:3].T, c = cpos_p[:,0], s=10, cmap = 'magma')
            ax.scatter(*aligned_cemb_r[:,:3].T, c = cpos_r[:,0], s=10, cmap = 'magma')
            ax.set_aspect('equal', adjustable='box')
            ax = plt.subplot(3,3,9, projection = '3d')
            ax.scatter(*cemb_p[:,:3].T, color = dir_color_p, s=10)
            ax.scatter(*aligned_cemb_r[:,:3].T, color = dir_color_r, s=10)
            ax.set_aspect('equal', adjustable='box')
            plt.tight_layout()
            fig.canvas.manager.window.showMaximized() # toggle fullscreen mode
            plt.savefig(os.path.join(save_dir,f'{mouse}_rotation_{emb}.jpg'), dpi = 400,bbox_inches="tight",transparent=True)
            plt.savefig(os.path.join(save_dir,f'{mouse}_rotation_{emb}.svg'), dpi = 400,bbox_inches="tight",transparent=True)
            plt.close(fig)
            rot_error_dict[emb] = {
                #original data
                'emb_p': emb_p,
                ...
                'cpos_p': cpos_p,
                'cpos_r': cpos_r,
                'cmov_dir_p': cmov_dir_p,
                'cmov_dir_r': cmov_dir_r,
                #centroids
                'cent_pre': cent_pre,
                'cent_rot': cent_rot,
                ...
                'cent_dir': cent_dir,
                #planes
                'norm_vector_p': norm_vector_p,
                'cloud_center_p': cloud_center_p,
                'xx_p': xx_p,
                'yy_p': yy_p,
                'zz_p': zz_p,
                'norm_vector_r': norm_vector_r,
                'cloud_center_r': cloud_center_r,
                'xx_r': xx_r,
                'yy_r': yy_r,
                'zz_r': zz_r,
                #alignment
                'Ralign': Ralign,
                'aligned_norm_vector_r': aligned_norm_vector_r,
                'cloud_center_r': cloud_center_r,
                'axx_r': axx_r,
                'ayy_r': ayy_r,
                'azz_r': azz_r,
                'aligned_cent_rot': aligned_cent_rot,
                'aligned_cemb_r': aligned_cemb_r,
                'angle_of_alignment': angle_of_alignment,
                #rotation
                'angles': angles,
                'mid_pre': mid_pre,
                'mid_rot': mid_rot,
                ...
                'rot_angle': rot_angle,
                #aligned rotation
                'aligned_mid_rot': aligned_mid_rot,
                'aligned_error': aligned_error,
                'aligned_norm_error': aligned_norm_error,
                'aligned_rot_angle': aligned_rot_angle
            }
...
for mouse in mice_list:
    ...
    for case in ['veh', 'CNO']:
        ...
        for emb in ['umap','isomap','pca']:
            cent_pre = rot_error_dict[emb]['cent_pre']
            cent_rot = rot_error_dict[emb]['cent_rot']

            cloud_center_p = rot_error_dict[emb]['cloud_center_p']
            cloud_center_r = rot_error_dict[emb]['cloud_center_r']

            inter_dist = np.linalg.norm(cloud_center_p-cloud_center_r)
            intra_pre = np.percentile(pairwise_distances(cent_pre),95)
            intra_rot = np.percentile(pairwise_distances(cent_rot),95)
            remap_dist = 2*inter_dist/np.max((intra_pre, intra_rot))
            print(f"\t{emb}: {remap_dist:2f}")


            aligned_cent_rot = rot_error_dict[emb]['aligned_cent_rot']
            plt.figure()
            ax = plt.subplot(1,1,1, projection = '3d')
            ax.scatter(*cent_pre[:,:3].T, color ='b', s=10)
            ax.scatter(*cent_rot[:,:3].T, color ='r', s=10)
            ax.scatter(*aligned_cent_rot[:,:3].T, color ='orange', s=10)
            ax.scatter(*cloud_center_p.T, color ='k', s=30)
            ax.scatter(*cloud_center_r.T, color ='k', s=30)
            ax.plot([cloud_center_p[0], cloud_center_r[0]],
                    [cloud_center_p[1], cloud_center_r[1]],
                    [cloud_center_p[2], cloud_center_r[2]], 'k')
            ax.set_aspect('equal', adjustable='box')
            plt.savefig(os.path.join(save_dir,f'{mouse}_{case}_distance_{emb}.jpg'), dpi = 400,bbox_inches="tight",transparent=True)
            plt.savefig(os.path.join(save_dir,f'{mouse}_{case}_distance_{emb}.svg'), dpi = 400,bbox_inches="tight",transparent=True)
...
view_init_dict = {
    'ChRNA7Charly1_veh': [-60,-130],
    'ChRNA7Charly1_CNO': [-160, -130],

    'ChRNA7Charly2_veh': [-109,-170],
    'ChRNA7Charly2_CNO': [-48, 6]
}
save_dir = os.path.join(base_dir, 'emb_examples')
...
for mouse in mice_list:
    ...
    for case in ['veh','CNO']:
        ...
        noise_idx_pre = rot_error_dict['umap']['noise_idx_pre']
        emb_p = rot_error_dict['umap']['emb_p'][~noise_idx_pre,:]
        pos_p = rot_error_dict['umap']['pos_p'][~noise_idx_pre,:]
        mov_dir_p = rot_error_dict['umap']['mov_dir_p'][~noise_idx_pre]

        noise_idx_rot = rot_error_dict['umap']['noise_idx_rot']
        emb_r = rot_error_dict['umap']['emb_r'][~noise_idx_rot,:]
        pos_r = rot_error_dict['umap']['pos_r'][~noise_idx_rot,:]
        mov_dir_r = rot_error_dict['umap']['mov_dir_r'][~noise_idx_rot]
        ...
        fig.suptitle(mouse)
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir,f"{mouse}_{case}_umap_emb.svg"), dpi = 400,bbox_inches="tight")
        plt.savefig(os.path.join(save_dir,f"{mouse}_{case}_umap_emb.png"), dpi = 400,bbox_inches="tight")
...
for mouse in mice_list:
    for case in ['veh','CNO']:
        ...
        rotation_list.append(rot_error_dict['umap']['aligned_rot_angle'])
...
ax.set_ylim([0,180])
plt.tight_layout()
plt.savefig(os.path.join(save_dir,f'chRNA7_rotation_boxplot.jpg'), dpi = 400,bbox_inches="tight",transparent=True)
plt.savefig(os.path.join(save_dir,f'chRNA7_rotation_boxplot.svg'), dpi = 400,bbox_inches="tight",transparent=True)
...
plt.tight_layout()
plt.savefig(os.path.join(save_dir,f'chRNA7_distance_barplot.jpg'), dpi = 400,bbox_inches="tight",transparent=True)
plt.savefig(os.path.join(save_dir,f'chRNA7_distance_barplot.svg'), dpi = 400,bbox_inches="tight",transparent=True)
...
for mouse in mice_list:
    for case in ['veh','CNO']:
        ...
        case_list.append(case)
        mouse_list.append(mouse)
        rotation_list.append(rot_error_dict['umap']['angle_of_alignment'])
...
ax.set_ylim([0,90])
plt.tight_layout()
plt.savefig(os.path.join(save_dir,f'chRNA7_alignment_boxplot.jpg'), dpi = 400,bbox_inches="tight",transparent=True)
plt.savefig(os.path.join(save_dir,f'chRNA7_alignment_boxplot.svg'), dpi = 400,bbox_inches="tight",transparent=True)
