function create_tracesEvents_struct_JP(mouse_name, varargin)
    ...
    folder_data = je.Results.folder_data;
    save_data = je.Results.save_data;
    session_names = je.Results.session_names;
    session_number = je.Results.session_number;
    ...
    files = dir(folder_data);
     if sum(contains(session_names, 'LT','IgnoreCase',true))*sum(contains(session_names, 'Rot','IgnoreCase',true)) == 1
         session_type = 'LT_ROT';
         fprintf('LT to Rotation pipeline selected.')
     elseif sum(contains(session_names, 'LT','IgnoreCase',true))*sum(contains(session_names, 'ALO','IgnoreCase',true))*...
             sum(contains(session_names, 'RL_EAST','IgnoreCase',true))*sum(contains(session_names, 'RL_WEST','IgnoreCase',true))== 1
    ...
    if length(session_names)>1
        if contains(session_type, 'LT_ROT', 'IgnoreCase',true)
            ...
            if ~any(correspondence_file_index)
                error("Couldn't find correspondance file. Please check it is on the folder with the word 'correspondace' in the name.")
...
function [tracesEvents] = get_tracesEvents(files,local_to_global_guide, condition)
    ...
    if isfield(tracesEvents,'droppedFrames')
        if ~isempty(tracesEvents.droppedFrames)
            ...
            tracesEvents.spikes(tracesEvents.droppedFrames,:) = [];
            tracesEvents.spikes_amp(tracesEvents.droppedFrames,:) = [];
    ...
    if length(unique([size(tracesEvents.position,1),size(tracesEvents.traces,1), ...
            size(tracesEvents.spikes,1),size(tracesEvents.events,1)]))>1
