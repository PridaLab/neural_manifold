...
def decoders_noise_1D(x_base_signal = None, y_signal_list=None, emb_list = [], nn = None, 
    ...
    for emb in ['base_signal', *emb_list]:
        R2s[emb] = dict()
        for decoder_name in decoder_list:
            R2s[emb][decoder_name] = np.zeros((n_splits,n_noise,len(y_signal_list),2))
            
    n_x_signals = len(['base_signal', *emb_list])
    predictions = [np.zeros((n_splits,n_noise,y.shape[0],n_x_signals+2)) for y in y_signal_list]
    for y_idx, y in enumerate(y_signal_list):
    ...
    for kfold_idx in range(n_splits):
        ...
        for noise_idx in range(n_noise):
            ...
            for y_idx, y in enumerate(y_signal_list):
                predictions[y_idx][kfold_idx,noise_idx,train_index,0] = 0
                predictions[y_idx][kfold_idx,noise_idx,test_index,0] = 1

            X_train = []
            X_base_train = nx_base_signal[train_index,:]
            X_train.append(X_base_train)
            X_test = []
            X_base_test = nx_base_signal[test_index,:]
            X_test.append(X_base_test)
            Y_train = [y[train_index,:] for y in y_signal_list]
            Y_test = [y[test_index,:] for y in y_signal_list]
            #compute embeddings
            ...
            for emb_idx, emb in enumerate(['base_signal', *emb_list]):
                for y_idx in range(len(y_signal_list)):
                    for decoder_name in decoder_list:
                        ...
                        #store results
                        R2s[emb][decoder_name][kfold_idx,noise_idx,y_idx,0] = test_error
                        R2s[emb][decoder_name][kfold_idx,noise_idx,y_idx,1] = train_error

                        total_pred = np.hstack((train_pred, test_pred)).T
                        total_pred = total_pred[np.argsort(np.hstack((train_index, test_index)).T)]
                        predictions[y_idx][kfold_idx,noise_idx,:,emb_idx+2] = total_pred
...
@gu.check_inputs_for_pd
def decoders_1D_shifted(x_base_signal = None, y_signal_list=None, emb_list = [], nn = None,
                    trial_signal = None, decoder_list = ["wf", "wc", "xgb", "svr"],
    ...
    min_shift = 100
    max_shift = x_base_signal.shape[0] - min_shift
    for kfold_idx in range(n_splits):
        if verbose:
            print(f"{pre_del}{kfold_idx+1}/{n_splits}", sep = '', end='')
            pre_del = (len(str(kfold_idx+1))+len(str(n_splits))+1)*'\b'
        ...
        neuron_shift = np.random.randint(min_shift, max_shift, x_base_signal.shape[1])
        x_base_signal_shuffled = np.zeros(x_base_signal.shape)
        for neu in range(x_base_signal.shape[1]):
            shift = neuron_shift[neu]
            x_base_signal_shuffled[:-shift,neu] = copy.deepcopy(x_base_signal[shift:,neu])
            x_base_signal_shuffled[-shift:,neu] = copy.deepcopy(x_base_signal[:shift,neu])
            
        X_train = []
        X_base_train = x_base_signal_shuffled[train_index,:]
        X_train.append(X_base_train)
        X_test = []
        X_base_test = x_base_signal_shuffled[test_index,:]
        X_test.append(X_base_test)
